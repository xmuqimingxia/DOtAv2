...
class Tracker3D:
    ...
    def compute_cost_map(self):
        ...
        whl_dis = np.abs(all_detections[...,9:11]-all_predictions[...,9:11])/(all_detections[...,9:11]+all_predictions[...,9:11]+0.00001)
        #print(whl_dis)
        #input()

        angle_dis = 1-np.cos(all_detections[...,11]-all_predictions[...,11])
        whl_dis = whl_dis.sum(-1)

        box_cost = (dis+0.1*whl_dis+1*angle_dis) * all_predictions[...,-1]
    ...
    def post_processing(self, config, pose = None):
        """
        ...
        for key in self.dead_trajectories.keys():
            ...
            track.filtering(config, pose = pose)
            tra[key] = track
        for key in self.active_trajectories.keys():
